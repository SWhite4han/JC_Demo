...
def get_symbol(args, arg_params, aux_params):
  ...
  elif args.loss_type==8: #centerloss
    fc7 = mx.sym.FullyConnected(data=embedding, weight = _weight, no_bias = True, num_hidden=args.num_classes, name='fc7')
#################################
    center_loss_ = mx.sym.Custom(data=embedding, label=gc_label, name='center_loss_', op_type='centerloss',\
            num_class=args.num_classes, alpha=0.5, scale=0.008, batchsize=args.per_batch_size)
    center_loss = mx.sym.MakeLoss(name='center_loss', data=center_loss_)
...
########################
  ...
  if args.loss_type==8:
    softmax_center = mx.symbol.Group([softmax, center_loss])
  out_list.append(softmax_center)
  if args.ce_loss:
