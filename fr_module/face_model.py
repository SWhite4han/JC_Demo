...
import cv2
import sklearn
from fr_module.mtcnn_detector import MtcnnDetector
sys.path.append(os.path.join(os.path.dirname(__file__), '..', 'src', 'common'))
from fr_module.src.common import face_preprocess
...
class FaceModel:
  def __init__(self, args):
    ...
    self.flip = args.flip
    if len(args.model)>0:
      self.model = get_model(ctx, image_size, os.path.abspath(os.path.join(os.path.curdir, args.model)), 'fc1')
  ...
  def get_multi_input(self, face_img):
    ...
    for i in range(len(bbox)):
      if bbox[i, 3] - bbox[i, 1] < 25:
        continue
      if bbox[i, 2] - bbox[i, 0] < 25:
        continue
      if bbox[i, 4] < 0.91:
        continue
      # print(bbox[i, 4])
      nimg = face_preprocess.preprocess(face_img, bbox[i, 0:4], points[i, :].reshape((2, 5)).T, image_size='112,112')
      nimg = cv2.cvtColor(nimg, cv2.COLOR_BGR2RGB)
      aligned.append(np.transpose(nimg, (2,0,1)))
    return aligned, bbox[:, 0:4]
