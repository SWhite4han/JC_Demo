...
class InfinitySearch:
    def __init__(self, conn_str):
        ...
        self.api_url = 'http://192.168.1.88:9527'
    ...
    def query_face_result(self, file_path, top=10, method='im_cosine', target_index='infinity'):
        """
        task: 0: yolo + facenet
              1: imagenet
              2: ocd + ocr
        ...
        if rlt_data and rlt_data['response'] == 'good':
    ...
    def query_image_result(self, file_path, top=10, method='im_cosine', target_index='infinity'):
        """
        task: 0: yolo + facenet
              1: imagenet
              2: ocd + ocr
        ...
        rlt = super_post(data=post_data, url=self.api_url)
        rlt_data = json.loads(rlt.text)
        if rlt_data and len(rlt_data['imgVec']) == 2048:
            json_query = json.dumps(
                {
                    "from": 0, "size": top,
                    "_source": ["imgPath", "category"],
